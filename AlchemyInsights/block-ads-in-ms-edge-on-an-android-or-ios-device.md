---
title: "\"Microsoft Edge\" skelbimų blokavimas \"Adroid\" arba \"iOS\" įrenginyje"
ms.author: v-aiyengar
author: AshaIyengar21
manager: dansimp
ms.date: 12/03/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "9003856"
- "6906"
ms.openlocfilehash: 8ad1e8dd59712de0be46c90bee24b0e8d290906f
ms.sourcegitcommit: c069f1b53567ad14711c423740f120439a312a60
ms.translationtype: MT
ms.contentlocale: lt-LT
ms.lasthandoff: 12/04/2020
ms.locfileid: "49573564"
---
# <a name="block-ads-in-microsoft-edge-on-an-adroid-or-ios-device"></a>"Microsoft Edge" skelbimų blokavimas "Adroid" arba "iOS" įrenginyje

Norėdami blokuoti visus skelbimus, išskyrus tuos, kurie "AdBlock Plus" laikomi priimtinais:
1. Pasirinkite **...** > **Parametrai**  >  **Turinio blokatoriai**.
2. Įjunkite blokuoti skelbimus.

Norėdami blokuoti visus skelbimus:
1. Pasirinkite **...** > **Parametrai**  >  **Turinio blokatoriai**.
2. Įjunkite blokuoti skelbimus.
3. Pasirinkite **Išplėstiniai parametrai**.
4. Išjunkite leisti priimtinus skelbimus.