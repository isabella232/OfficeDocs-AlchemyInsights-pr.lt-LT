---
title: 9000240 Outlook.com atsakymai
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "1825"
- "9000240"
ms.openlocfilehash: 2d9c7ee84cf793bb8181c8b66f8fc7faadba725a
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: lt-LT
ms.lasthandoff: 09/14/2020
ms.locfileid: "47723363"
---
# <a name="replying-in-outlookcom"></a>Atsakymas į "Outlook.com"

Norėdami atsakyti į vieną pranešimą vienu metu:

1. Pranešimų sąraše pasirinkite pranešimą, į kurį norite atsakyti.
2. Pranešimų srities viršuje dešinėje pasirinkite <img src='data:image/png;base64,' /> **Atsakyti** arba <img src='data:image/png;base64,' /> **Atsakyti visiems**.
3. Įveskite pranešimą ir pasirinkite **siùsti**.

Norėdami automatiškai atsakyti visiems pranešimams:

1. eiti į <img src='data:image/png;base64,' />
**Parametrai**  >  **Peržiūrėti visus "Outlook" parametrus**  >  **Paštas**  >  **Automatiniai atsakymai** , kad atidarytumėte [automatinio atsakymo parametrus](https://outlook.live.com/mail/options/mail/automaticReplies).
2. Pasirinkite perjungiklį **Įjungti automatinius atsakymus**.
3. Pažymėjus žymės langelį **išsiųsti atsakymus tik per** laikotarpį, galima:
    - Siųskite atsakymus tik tada, kai pasirenkate. Jei nenustatėte laikotarpio, Automatinis atsakymas liks įjungtas, kol išjungsite.
    - Kalendoriaus blokavimas
    - Automatiškai atmesti naujus kvietimus
    - Atmesti ir atšaukti mano susitikimų šiuo laikotarpiu
4. Lauke pranešimas įveskite pranešimą, kurį norite išsiųsti žmonėms tuo metu, kai esate išvykę.
5. Jei norite, kad atsakymai būtų siunčiami tik kontaktams, pažymėkite žymės langelį **išsiųsti atsakymus tik kontaktams** .
6. Pasirinkite **įrašyti**.

Sužinokite daugiau apie [automatinį peradresavimą "Outlook.com](https://support.office.com/article/14614626-9855-48dc-a986-dec81d07b1a0?wt.mc_id=Office_Outlook_com_Alchemy)".